...
import sys
import shutil
from enhance_utils import generate_blur, generate_blur_motion, gaussian_noise, generate_photo_light, generate_photo_shadow, check_image_type
...
def process_image(file):
    ...
    if file_name_ext == '.jpg' or file_name_ext == '.png':
        ...
        if 'N' in enhance_mark:
            if is_scan_image:
                im = gaussian_noise(im,20)
            else:
                im = gaussian_noise(im,10)

            #im = generate_noisy(im, noise_level=0.01, max_noise_size=3)
        if 'L' in enhance_mark:
            if is_scan_image:
                im = generate_photo_light(im,direction=-1, darkness=0.7)
            else:
                im = generate_photo_light(im,direction=-1, darkness=0.3)

        if 'S' in enhance_mark:
            if is_scan_image:
                im = generate_photo_shadow(im, darkness=0.7)
            else:
                im = generate_photo_shadow(im, darkness=0.3)
